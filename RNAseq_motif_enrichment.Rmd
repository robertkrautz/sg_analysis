...
##------------------------------------##
##----Comparison_of_conditions_all----##
##------------------------------------##

## [3.0.] Automated read-out of gene lists
...
## [3.2.] Read out all gene occurences
```{r}
tmp <- res_genos %>%
  tidyr::unnest(genes) %>%
  dplyr::distinct(genes)

for(i in 1:base::nrow(options)){
...
## [3.3.] Summarise distributions
```{r}
cols <- res_distr %>%
  dplyr::select(-genes) %>%
  base::colnames()

res_distr_sum <- res_distr %>%
  dplyr::group_by_at(cols) %>%
  dplyr::summarise(
      n = dplyr::n(),
      g = str_c(genes,collapse = ";")
    )
```

## [3.4.] Insert other combinations into res_list
```{r}
res_list[["RasV12lglRNAiRasV12_down_incl"]] <- res_distr %>%
  dplyr::filter(
      RasV12_down == 1L &
      lglRNAiRasV12_down == 1L
    ) %>%
  dplyr::pull(genes)

res_list[["RasV12lglRNAiRasV12_up_incl"]] <- res_distr %>%
  dplyr::filter(
      RasV12_up == 1L &
      lglRNAiRasV12_up == 1L
    ) %>%
...
res_list[["lglRNAiRasV12_FM_down_excl"]] <- res_distr %>%
  dplyr::filter(
      RasV12_down == 0L &
      lglRNAiRasV12_down == 0L &
      lglRNAiRasV12_FM_down == 1L
    ) %>%
  dplyr::pull(genes)

res_list[["lglRNAiRasV12_FM_up_excl"]] <- res_distr %>%
  dplyr::filter(
      RasV12_up == 0L &
      lglRNAiRasV12_up == 0L &
      lglRNAiRasV12_FM_up == 1L
    ) %>%
  dplyr::pull(genes)

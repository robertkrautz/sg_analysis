---
title: "20190706_workflow_NG8133_RNAseq"
...
## [3.3.] Customized PCA plot data preparation (see [0.2.])
#### 'est_counts' for transcript-centric modelling
#### 'scaled_reads_per_base' for gene-centric modelling
#### 'sout$obs_norm_filt' also includes tpm values
```{r}
...
for(ext in c("pdf", "png")){
  ggsave(
    filename = paste0(
        here("results/20190706_NG8133_RNAseq/"),
        format(Sys.time(), "%Y%m%d"),
        "_customized_pca_plot_allConditions",
        ".", ext
        ),
    plot = pca,
    device = ext,
...
#### help page for sleuth_results is not accurately stating this
#### evaluate 'res_wt_viz %>% group_by(clring) %>% summarise(sum = n())'
#### previous test: "RasV12"
```{r}
...
for(ext in c("pdf", "png")){
  ggsave(
    filename = paste0(
        here("results/20190706_NG8133_RNAseq/"),
        format(Sys.time(), "%Y%m%d"),
        "_volcano_lglRNAiRasFM_vs_W1118_waldTest",
        ".", ext
        ),
    plot = vol,
    device = ext,
...
## [5.4.] Parallel implementation of hyperGTest() on all combinations
#### not working as of yet
```{r eval=FALSE}
allGO <- foreach(i=1:nrow(res_genes), .packages = c("GOstats","DOSE")) %dopar% {
    
    cur <- res_genes %>%
      dplyr::filter(row_number() == i)
  
    ont <- dplyr::pull(cur,ontology)
    sig <- dplyr::pull(cur,significance)
    ens <- dplyr::pull(cur,ensembl) %>% unlist
    ent <- dplyr::pull(cur,entrez) %>% unlist
    ...
    return(
      out %>%
        dplyr::rename(.,ID=1) %>% 
        dplyr::mutate(
          condition = dplyr::pull(cur,condition),
          filter = dplyr::pull(cur,filter),
          ontology = ont,
          significance = sig
        ) %>%
        dplyr::select(8:11,1,7,2,5:6,3:4)
      )
  }
```

## [5.5.] Serial implementation of hyperGTest() on all combinations
```{r}
allGO <- res_genes %>%
  dplyr::mutate(
    go = purrr::pmap(
...
## [5.7.] Read out patterns for GOs of interest
```{r}
patterns <- c(
    "immune", "defense", "Toll",
...
## [5.8] clusterProfiler::enrichGO() as alternative to GOstats::hyperGTest()
```{r eval=FALSE}
...
## [7.5.] Subset res_all for immune genes of "GO:0006955"
```{r}
 GOI <- c("RasV12","lglRNAiRasV12","lglRNAiRasV12_FM")
...
for(ext in c("pdf", "png")){
  ggsave(
    filename = paste0(
        here("results/20190706_NG8133_RNAseq/"),
        format(Sys.time(), "%Y%m%d"),
        "_lglRNAiRasV12_FM_genes.immune",
        ".", ext
        ),
    plot = imm,
    device = ext,
...
#### [7.7.] Save upseter plot
```{r}
pdf(
  file = paste0(
      here("results/20190706_NG8133_RNAseq/"),
      format(Sys.time(), "%Y%m%d"),
      "_upset_intersectConditions",
      ".pdf"
    ),
  onefile = TRUE,
  width = 6L,
...
## [8.2.] Identify lglRNAiRasV12_FM specific genes & convert 
```{r}
...
## [8.3.] Perform GO term enrichment
```{r}
tmp <- new(
    'GOHyperGParams',
...
## [8.4.] Read out results for "GO:0006955"
```{r}
out %>% 
  summary(., pvalue = 1) %>%
